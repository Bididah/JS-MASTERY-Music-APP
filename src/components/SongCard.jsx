...
const SongCard = ({song, isPlaying, activeSong, i, data}) => {
  ...
  <div className="flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slidup rounded-lg cursor-pointer">
    <div className="relative w-full h-56 group">
      <div className={`absolute inset-0 justify-center items-center bg-black bg-opacity-50 group-hover:flex ${ activeSong?.title === song ? 'flex bg-black bg-opacity-70': 'hidden'}`}>
        <PlayPause 
      song = {song}
      isPlaying = {isPlaying}
